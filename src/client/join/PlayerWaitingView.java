...
public class PlayerWaitingView extends OverlayView implements IPlayerWaitingView {
	...
	public void setPlayers(ArrayList<Player> value) {
		...
		for (int i = 0; i < value.size(); i++) {
			if (value.get(i) != null) {
				System.out.println("SETTING NEW PLAYERS");
				String builtString = (i + 1) + " " + value.get(i).getName();
				...
				playerPanel.setBackground(CatanColor.toColor(value.get(i).getColor()).getJavaColor()); // set
																										// the
																										// background
																										// color
																										// of
