...
public class ResourceBarController extends Controller implements IResourceBarController {
	...
	public boolean abletobuildroad() {
		...
		if (playerinventory.getResources().getWood() < 1){
			...
			return false;
		}else if(!(currentplayer == thisplayerindex) &&!(status.equals("Playing"))&&!((playerinventory.getRoads() <= 15))) {
			return false;
		}else{
			return true;
		}
	...
	public boolean abletobuildsettlement() {
		...
		if (playerinventory.getResources().getWood() < 1){
			...
			return false;
		}else if(!(currentplayer == thisplayerindex) &&!(status.equals("Playing"))&&!((playerinventory.getSettlements() <= 5))) {
			return false;
		}else{
			return true;
		}
